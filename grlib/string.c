...
void
GrRyansStringDraw(tContext *pContext, const char *pcString, int32_t i32Length,
             int32_t i32X, int32_t i32Y, uint32_t bOpaque)
{
    ASSERT(pContext);
    ASSERT(pContext->pfnStringRenderer);
    ...
    // Call the currently registered string rendering function.
    //
    GrRyansStringRenderer(pContext, pcString, i32Length, i32X, i32Y,
                                bOpaque);
}
...
// Doin' it my way!
void
GrRyansStringRenderer(tContext *pContext, const char *pcString,
                        int32_t i32Length, int32_t i32X, int32_t i32Y,
                        bool bOpaque)
{
    uint8_t ui8Format, ui8Width, ui8MaxWidth, ui8Height, ui8Baseline;
    ...
    while(ui32Count)
    {
        ...
        // If we are already outside the clipping region, exit early.
        if(pContext->sStringPoint.i16X >= (pContext->sClipRegion.i16XMax - 55) )
        {
        	pContext->sStringPoint.i16Y += ui8Height+2;

            if(pContext->sStringPoint.i16Y >= pContext->sClipRegion.i16YMax)
            	return;
            else
            	pContext->sStringPoint.i16X = 0;
        }
        ...
        if(pui8Data)
        {
            GrFontGlyphRender(pContext, pui8Data, pContext->sStringPoint.i16X, pContext->sStringPoint.i16Y,
                              (ui8Format & FONT_FMT_PIXEL_RLE) ? true : false,
                              bOpaque);
            pContext->sStringPoint.i16X += ui8Width;
        }
        else
        {
            // Leave a space in place of the undefined glyph.
        	pContext->sStringPoint.i16X += ui8MaxWidth;
        }
